...
- Content policy (amended 2026-07-18): OKF documents contain original synthesis
  ...
  status (`open`, `identified`, `ingested`). The application identifies what
  it might need (`identified`), and the user ingests it (`ingested`). Deeper use of a
  work (excerpts, adaptation) is unlocked when its status reaches `ingested`
  or `open`; until then documents cite and link only.
